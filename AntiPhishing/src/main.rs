...
5. 合法系統與資安通報排除：若郵件安全驗證狀態為通過（如 SPF/DKIM/DMARC pass 或信任來源），且內容屬於企業資安通報、Hinet/防毒系統之病毒攔截通報、郵件伺服器之垃圾信隔離清單彙整等例行性系統報告，屬於正常業務郵件，is_phishing 必須為 false（不可因內文提及或列出被攔截的惡意網址與標題而誤判）。\
\
...
/// 郵件安全驗證解析結果（包含 SPF、DKIM、DMARC、TLS 以及偽造警示）
    ...
    for name in check_headers {
        for val in mail.headers.get_all_values(name) {
            ...
            if status.dmarc.is_none() {
                ...
                } else if lower.contains("dmarc=reject") {
                    status.dmarc = Some("reject".to_string());
            ...
            if status.dkim.is_none() {
                if lower.contains("dkim=pass") {
                    ...
                    status.dkim = Some("neutral".to_string());
