...
- **排程**：無獨立排程執行緒；`eframe::App::logic` 每幀 poll，`Instant >= next_check` 時觸發掃描（`request_repaint_after(1s)` 保持喚醒）。啟動時掃「前一日＋今日」，之後排程只掃今日。`receiver.is_some()` 護欄同時擋住 pending 確認期間的二次掃描（包含系統匣「立即掃描」）。
