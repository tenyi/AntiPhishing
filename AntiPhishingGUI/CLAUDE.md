...
- **搬移確認與佇列**：`[gui] confirm_before_move`（預設 true）開啟時，掃描發現之疑似郵件存入 `pending_queue`（持久化於 `scan_state.toml` 跨重啟保留），UI 彈出確認視窗（Modal Dialog）並於頂部顯示提醒橫條。使用者點擊「隔離選取郵件」時，啟動獨立短暫的搬移 Worker（`execute_move_task`）：重新建立乾淨連線 → 搬移所選 UID（具郵件不存在之容錯機制）→ `uid_expunge` → 立即登出關閉連線，並自佇列剔除已搬移項目；點擊「全部跳過」或「稍後處理」不阻塞背景排程。
